...
function admTopFolderClick(){
	...
	$("#adm_dataroom").bind("click", function(event){
		...
		if(localStorage._zs == "B") {
			...
			if (urlk.length > 1 && urlk[1].indexOf("a=view") == 0) {
				...
				foldrType = arr[2].replace("c=", "");
			}
			if(foldrType != undefined && foldrType.length > 0){
				fetchAllFolderListDetails(foldrType, true);
			}else{
	...
	unbindobject("#imgrmexe");
	$("#imgrmexe").bind("click", function(event){
		admShowconfirmmsg(adm_messages.admdrmexeinstall, confirm_Info, 5000, "", false, false);
		window.open(cloudURLACT+"/DCirrusDRM.zip", "_blank");
	});
}
...
function admFolderClickMoreEvent(id) {
	...
	$("#"+id).click(function(){
		...
		if(idm == "adm_header_-1"){
			if (this.innerText == "Search >> " || this.innerText == "Search") {
				$("#searchdone").click();
			} else {
	...
	$("#"+id).click(function(){
		...
		if(allowdn) {
			if(dowloadfolderidsui[folderid] == undefined || dowloadfolderidsui[folderid] == null){
				...
				$("#imgclassid_"+idm).addClass("respons");
				$("#drm_folder_info_"+folderid).css("margin-top", "28px");
				$("#adm_foldername_"+idm).css("margin-top", "");
...
function admPagesCountCheckbox() {
	admCloseRowDropDown();
	var strfiles = "";
...
function admHeaderCheckboxChecked() {
	...
	$("#admallcheckbox").click(function () {
		admPagesCountCheckbox();
	});
	if ($("#admallcheckbox").is(':checked')) {
		$("#admallcheckbox").prop('checked', false);
		$(".admcheckbox").prop("checked", false);
		$("#admallcheckbox").trigger('click');
	}
	admPagesCountCheckbox();
}
...
function admDownloadCommon(){
	...
	$("#adm_download_file").click(function() {
		...
		if(admCheckIfOneDocumentSelected()){
			...
			var multiSelectFile = "";
			if(adm_search_progress)multiSelectFile = "download";
			var jsonInput = {
				attribute1:dcids,
				attribute2:1,
				attribute3:1,
				attribute4:0,
				attribute5:multiSelectFile,
...
function admOpenDeleteCommon(){
	...
	unbindobject("#adm_drm_setting");
	$("#adm_drm_setting").click(function() {
		admCloseRowDropDown();
		if(admCheckIfOneDocumentSelected()){
			var fileNames = "";
			$('input.admcheckbox:checked').each(function(i, obj) {
				fileNames += "<div>"+$(this).attr("data-filename")+"</div>";
			});
			$("#drm_file_folder_list").html(fileNames);
			admDefaultDRMpopup("Files");
			$("#modaldrmfile").attr("data-fileid", "multi");
		}
	});
}
...
function admfetchprojectIndexEvent(){
	...
	unbindobject("#filecontentsearch");
	$("#filecontentsearch").click(function() {
		admCloseRowDropDown();
		admshowAIPopup = true;
		admCheckSearchIndexService();
	});

	unbindobject("#btncontentsearchclose");
	$("#btncontentsearchclose").click(function() {
		hideActionPopup("divcontentsearchmodal");
	});

	unbindobject("#btncontentsearch");
	$("#btncontentsearch").click(function() {
		if($("#txt_search_index").val().trim().length > 0){
			admFetchSearchIndexService();
			hideActionPopup("divcontentsearchmodal");
		} else {
			admShowconfirmmsg("Enter content to be searched", confirm_Error, 5000, "", false, false);
		}
	});

	unbindobject("#btn_content_sync_now");
	$("#btn_content_sync_now").click(function() {
		admSyncNowSearchIndexService();
	});

	unbindobject("#btncontentsearchrequest");
	$("#btncontentsearchrequest").click(function() {
		admSearchIndexRequestService();
	});

	$("#divcontentsearchmodal").on('hide.bs.modal', function(){
		//$("#txt_search_index").val("");
	});
}
...
function admaddtagpopupopen(id){
	...
	$("#"+id).bind("click", function() {
		...
		if(locked == 0){
			...
			var ai = document.getElementById(this.id).getAttribute("data-ai");
			var status = document.getElementById(this.id).getAttribute("data-status");
			if(status != "D"){
				var docid = document.getElementById(this.id).getAttribute("data-id");
				var filename = document.getElementById(this.id).getAttribute("data-filename");
				if(ai == "true" && $("#docinsight_"+idm).hasClass("dinsight")){
					$("#adm_filenm_"+idm).hide();
					$("#adm_filenm_progress_"+idm).show();
					admFetchAIDocumentService(docid, filename, idm);
				} else {
					$("#tbltagrows").html("");
					$("#admtagnewtxt").val("");
					admFetchTags(docid);
...
function admUsernameClickTop(){
	...
	unbindobject("#adm_qrcode");
	$("#adm_qrcode").bind("click",function(event){
		showActionPopup("divauthregeneratemodal");
		$("#adm_regen_authenticator_set").hide();
		$("#btn_regenerate_auth").hide();
		$("#adm_regen_authenticator_pass").show();
		$("#btn_regenerate_pass").show();
	});

	unbindobject("#btnotpregeneratecancel");
	$("#btnotpregeneratecancel").bind("click",function(event){
		$('#txt_pass_auth').val('');
		hideActionPopup("divauthregeneratemodal");
	});

	unbindobject("#btn_regenerate_auth");
	$("#btn_regenerate_auth").bind("click",function(event){
		admRegenerateQRCodeService();
	});

	unbindobject("#btn_regenerate_pass");
	$("#btn_regenerate_pass").bind("click",function(event){
		if($("#txt_pass_auth").val().trim().length > 0){
			admCheckValidpassService();
			$('#txt_pass_auth').val('');
		} else {
			admShowconfirmmsg(adm_messages.enterpassword, confirm_Error, 5000, "", false, false);
		}
	});
}
...
function admDownloadStatusEvent(){
	...
	unbindobject("#downloadmodalminimize");
	$("#downloadmodalminimize").bind("click",function(event){
		var cleared = true;
		$(".downloadstatus").each(function() {
			if($(this).html() != "100%"){
				cleared = false;
...
function admSwitchAccountEvent(id){
	...
	$("#divuseraccount_signout_"+id).bind("click",function(event){
		...
		if(idm == uniqueidentifieracclist())location.reload();
	});
...
function admFilePermissionCheckBoxEvent(id){
	...
	$("#"+id).click(function() {
		...
		$("#adm_file_perm_popup_"+idm).addClass("adm_file_perm_checkbox_cls");
...
function admFilePermEvent(){
	...
	$("#btn_save_file_perm").click(function() {
		...
		if (objectData.UnIndexDocPermDtoList.length > 0)
			admSaveFilePermissionService(objectData);
		else{
			ajaxindicatorstop();
		admShowconfirmmsg("Please select any user.", confirm_Error, 5000, "", false, false);
	}
		return false;
	});
...
function admDRMFolderSetting(id){
	unbindobject("#adm_drm_settings_"+id);
	$("#adm_drm_settings_"+id).click(function(event){
		admCloseRowDropDown();
		admDefaultDRMpopup("Folder");
		$("#drm_file_folder_list").html("<div>"+$(this).attr("data-foldername")+"</div>");
		$("#modaldrmfile").attr("data-folderpath", $(this).attr("path"));
		$("#modaldrmfile").attr("data-folderid", $(this).attr("data-id"));
		return false;
	});
}
...
function admFileRowEvents(id){
	unbindobject("#admdocrowiconsdrm_"+id);
	$("#admdocrowiconsdrm_"+id).click(function(event) {
		event.stopPropagation();
		var index = (this.id).replace("admdocrowiconsdrm_", "");
		var idm = $(this).attr("data-id");
		$("#drm_file_folder_list").html("<div>"+$(this).attr("data-filename")+"</div>");
		admCloseRowDropDown();
		var locked = $("#adm_doc_row_"+index).attr("data-locked");
		var status = $("#adm_doc_row_"+index).attr("data-status");
		if(locked == 0 && status != "D"){
			var fileid = $("#adm_doc_row_"+index).attr("data-id");
			$("#modaldrmfile").attr("data-fileid", fileid);
			$("#modaldrmfile").attr("data-id", 0);
			$("#adm_doc_checkbox_"+index).prop("checked", true);
			admDRMFetchService(fileid, "");
		} else {
			if(locked == 1){
				admShowconfirmmsg(adm_messages.lockeddoc, confirm_Error, 5000, "", false, false);
			} else if(status == "D"){
	...
	unbindobject("#adm_drm_span_"+id);
	$("#adm_drm_span_"+id).click(function(event) {
		event.stopPropagation();
		return false;
	});
...
function admDRMEvents(){
	unbindobject("#adm_drm_close,#adm_drm_close_icon");
  	$("#adm_drm_close,#adm_drm_close_icon").click(function () {
		$(".admcheckbox").prop("checked", false);
		$("#admallcheckbox").prop("checked", false);
		$("#drm_file_folder_list").html("");
		$("#drm_selected_on").html("");
		hideActionPopup("modaldrmfile");
		return false;
	});

	unbindobject("#adm_drm_filter_update");
  	$("#adm_drm_filter_update").click(function () {
		if($("#txt_drm_expiry_date").val().trim().length > 0){
			var folderpath = $("#modaldrmfile").attr("data-folderpath");
			var expiryDate = $("#txt_drm_expiry_date").val();
			expiryDate = formatAddTZDate(expiryDate.replace("T"," "),true);
			var allowPrint = 0;
			if($("#adm_drm_allow_print").is(":checked")) allowPrint = 1;
			if(folderpath == ""){
				var fileId = $("#modaldrmfile").attr("data-fileid");
				var id = $("#modaldrmfile").attr("data-id");
				admDRMAddService(allowPrint, expiryDate, "");
			} else {
				var folderId = $("#modaldrmfile").attr("data-folderid");
				admDRMAddByFolderIdService(allowPrint, expiryDate, folderId, "");
			}
		} else {
			admShowconfirmmsg(adm_messages.admdrmexpirydate, confirm_Error, 5000, "", false, false);
		}
		return false;
	});

	unbindobject("#adm_drm_allow_all");
  	$("#adm_drm_allow_all").click(function () {
		if($("#modaldrmfile").attr("data-folderpath") == ""){
			var fileIds = "";
			var indexes = "-1";
			$('input.admcheckbox:checked').each(function(i, obj) {
				var idm = (this.id).replace("adm_doc_checkbox_", "");
				var docid = $(this).attr("data-id");
				if(fileIds.length == 0) fileIds = docid;
				else fileIds += "," + docid;
				indexes += ","+idm;
			});
			admDRMDeleteService(fileIds, "", indexes);
		} else {
			admDRMDeleteByFolderService($("#modaldrmfile").attr("data-folderid"));
		}
		return false;
	});

	unbindobject("#adm_drm_block_all");
  	$("#adm_drm_block_all").click(function () {
		var fileId = $("#modaldrmfile").attr("data-fileid");
		var d = new Date();
 		d.setDate(d.getDate()-365);
		var expiryDate = FormatDateToServer(d);
		var allowPrint = 0;
		var folderid = $("#modaldrmfile").attr("data-folderid");
		if(folderid == "0" || folderid == ""){
			admDRMAddService(allowPrint, expiryDate, "blockall");
		} else if(folderid > 0){
			admDRMAddByFolderIdService(allowPrint, expiryDate, folderid, "blockall");
		}
		return false;
	});
}
...
function permsearchEvent() {
	// Search all columns
	$('#txt-search-input-file').keyup(function () {
		permcommonsearch()
		// Search Text
		...
		if (admuserfilepermlist != null) {
			var attributearr = [];
			var chkVal = false;
			if ((search.trim()).length > 0) {
				chkVal = true;
				attributearr[0] = "loginidact<#HASH;>string<#HASH;>" + search;
			}
			var response = { "object": "", "error": false, "overwrite": 1 };
			if (chkVal == true) {
				response.object = searchJSONLowerCaseMatchMultiple(admuserfilepermlist, attributearr);
			} else {
				response.object = admuserfilepermlist;
			}
			searchdone = 1;
			displaycounter = -1;
			admfetchActiveLoginIdsFilePerm(response.object);
		}
	});
...
function ClearFields() {
	...
	document.getElementById("txt-search-input-file").value = "";
	$(".norecord").hide();
	$("#searchclear").hide();
}

function admUnlockPdfEvents(){
	unbindobject("#btnsendpdfpassword");
	$("#btnsendpdfpassword").bind("click", function (event) {
		if($("#txt_pdf_password").val().length == 0){
			admShowconfirmmsg(adm_messages.enterpdfpass, confirm_Error, 5000, "", false, false);
		} else if($("#txt_pdf_captcha").val().length == 0){
			admShowconfirmmsg(adm_messages.entercaptcha, confirm_Error, 5000, "", false, false);
		} else {
			$("#adm_filenm_"+$("#passwordIDM").val()).hide();
			$("#adm_filenm_progress_"+$("#passwordIDM").val()).show();
			admUnlockPdfService($("#txt_pdf_password").val(),$("#txt_pdf_captcha").val(), $("#passwordIDM").val());
			$("#txt_pdf_password").val("");
			$("#txt_pdf_captcha").val("");
			hideActionPopup("divaskpdfpassword");
		}
	});

	unbindobject("#btnsendpdfpasswordclose");
	$("#btnsendpdfpasswordclose").bind("click", function (event) {
		$("#adm_filenm_"+$("#passwordIDM").val()).show();
		$("#adm_filenm_progress_"+$("#passwordIDM").val()).hide();
		$("#txt_pdf_password").val("");
		$("#txt_pdf_captcha").val("");
		$("#passwordIDM").val("");
		hideActionPopup("divaskpdfpassword");
