	...
	*/
	...
	Tree.prototype.setSelectedState = function (node, state, options) {
		...
		if (state) {
			...
			if(prvselectednode != null){
				prvselectednode.state.selected = false;
				if (!options.silent) {
					this.$element.trigger('nodeUnselected', $.extend(true, {}, prvselectednode));
				}
			}
			node.state.selected = true;
			prvselectednode = node;
			if (!options.silent) {
		...
		$.each(nodes, function addNodes(id, node) {
			if(node.text.trim().length > 0){
				...
				if (_this.options.enableLinks) {
					...
					// otherwise just text
					var tag = "";
					if(node.tags != null && node.tags != undefined && node.tags.length > 0 && node.tags[0].length > 0){
						tag = node.tags[0]+"-";
					}
					var pm = node.text;
					var totalLength = 30;
					if((tag+pm).length >= totalLength){
						var charRemaining = totalLength - tag.length;
						var lfolderLength = pm.length;
						var fixedLength = "...".length;
						var displayChar = charRemaining - fixedLength;
						if(lfolderLength > displayChar){
						pm = pm.substring(0,displayChar)+"...";
						}	
					}
					treeItem.append(tag+pm);
					nodesList[node.id] = node.nodeId;
				}
	...
	Tree.prototype.template = {
		list: '<ul class="list-group" style="white-space: nowrap;overflow-x:auto;width:300%;float:left;"></ul>',
