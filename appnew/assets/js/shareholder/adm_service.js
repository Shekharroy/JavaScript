...
function fetchAllFolderListDetails(folderType) {
	try{
		var bs64 = getauthtokenfromlocal();
		if(bs64 != undefined && bs64 != null && bs64 != "" && bs64 != "null" && bs64 != "undefined") {
			admcheckiffromurl();
		}
	}catch(error){}
...
function admDisplayDocViewer(docId, idm, viewertype, folderId){
	...
	var extraparamjson = {"idm":idm,"nointernet":"admDisplayDocViewerAfter", "fdid":folderId};
	invokeAdapterCallFromAdm(type, posturl, "", "", extraparamjson);
}
...
function admDownloadFolderService(folderId, foldername, idm, org){
	...
	xhr.onreadystatechange = function (){
	    if (xhr.readyState === 4) {
			if(xhr.status == 200){
				...
				var blob = xhr.response;
				if (navigator.msSaveBlob) {
				  return navigator.msSaveBlob(blob, foldernm);
				}
				...
				admDeleteDownloadTempFolderService(filename);
	...
	xhr.onprogress = function(e) {
		...
		console.log("perc : " + perc + " :: initprec : " + initprec + " :: prv perc : " + prvperc +" :: 111 : " + (parseFloat(perc) > parseFloat(prvperc)));
...
function admDisplaySearchFilesService(){
	...
	if(filename.trim().length == 0 && filetype.trim().length == 0 && dtfrom == null && dtto == null && sizefrom == 0
		...
		var jsonInput = {
			...
			lock:lock,
			sign:sign,
			maxLimit:adm_documentmaxlimit,
			sorting:"DESC`lastmodified"
		};

		var extraparamjson = {"folderType":folderType};
		ajaxindicatorstart('loading data.. please wait..');
		var type = updateMethod;
		var posturl = cloudApiUrlACMS+admSearchURL;
