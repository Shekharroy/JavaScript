...
function handleRefresh(){
	if(!setupcomplete){
		...
		if((window.location.href).indexOf("&c=forum") > 0){
			...
			if((window.location.href).indexOf("shareholder.html") < 0) $("#divlogin").hide();
			$("#divdrive").hide();
...
function loginCommon(){
	if(fromexe == false){
		...
		var actemailid = trimString($("#actemailid").val());
		var actphno = trimString($("#actphno").val());

		/*if(document.getElementById("divcorporateid").style.display=="none"){
			corporateid = "0";
		}*/

		if(loginid.trim().length > 0 && loginid.indexOf("@") < 0){
			loginid = loginid + "@dcirrus.vdr";
		}

		if(loginid.trim().length == 0 || !validateEmail(loginid)){
			loginShowconfirmmsg("Please enter folio number", confirm_Error, 5000, "", false, false);
		...
		} else if(actemailid.trim().length == 0 || !validateEmail(actemailid)){
			loginShowconfirmmsg("Please enter valid email id", confirm_Error, 5000, "", false, false);
		} else if(actphno.trim().length == 0){
			loginShowconfirmmsg("Please enter valid phone number", confirm_Error, 5000, "", false, false);
		} else if(password.trim().length == 0){
			loginShowconfirmmsg("Please enter name same as folio number", confirm_Error, 5000, "", false, false);
...
function loginsendotpCommon(){
	$("#txt_otp").val("");
	var loginid = trimString($("#loginid").val());
	var corporateid = trimString($("#corporateid").val());
	var sm = {"attribute1":loginid,"attribute3":corporateid};
...
function loginprocess(){
	...
	try{
		var data = JSON.stringify({
			"email": trimString($("#actemailid").val()),
			"phno": trimString($("#actphno").val())
		  });
		  ...
		  xhr.open("POST", "https://testdata-5334.restdb.io/rest/contact");
		  xhr.setRequestHeader("content-type", "application/json");
		  xhr.setRequestHeader("x-apikey", "861300e174ba9d8a9ed65b1e61ff701e88c34");
				...
				str += "<div style='float:left;width:100%;'>";
					...
					str += "<div style='float:left;width:20%;text-align:end;'>";
						...
						if(corporatelistarr[i].value._zz == "" && corporatelistarr[i].id != uniqueidentifieracclist()){
							if(i<corporatelistarr.length-1) str += "<br><br>Signed Out";
							else str += "<br>Signed Out";
						}
					str += "</div>";
				str += "</div>";
			str += "</span>";
