...
function loginsuccess(response)
{
	...
	if(response != null)
	{
		...
		if(window.location.href.indexOf("/drive.html") > 0) {
			ajaxindicatorstop();
			if(responsecode == 415 && globalvariable.screenloadednew == false){
				...
				if (responsecode == 419){
					...
					showMessageErrorLogin(response.message, confirm_Error, true);
					...
					showMessageErrorLogin(login_messages.ipaddressblocked, confirm_Error, false);
				}else if (responsecode == 439){
					showMessageErrorLogin(login_messages.corporateInactive, confirm_Error, false);
