...
function fetchdeviceid(skde)
{
	...
	try{
		try
		{
			...
			var currentUserId = localStorage._zv;
			var currentLawfirmId = localStorage._zw;
			...
			var sm = {
				...
				"deviceDto":{
					...
					"regNew":"browser",
					"deviceId":_tdfrt,
					"tempAttr1": currentLawfirmId,
					"tempAttr2": currentUserId
				}
			};
...
function checkErrorPage(response){
	if((window.location.href).indexOf("/drive.html") > 0){
		if(response.messageCode == 437){
			ajaxindicatorstop();
			clearauthinlocalstorage();
			base64auth = "";
			loginShowconfirmmsg(global_messages.licenseexpired, confirm_Error, 5000, "", false, false);
			setTimeout(function(){window.location.href = "drive.html";}, 3000);
		}else if(response.messageCode == 422){
			ajaxindicatorstop();
			clearauthinlocalstorage();
			base64auth = "";
			loginShowconfirmmsg(login_messages.corporateExpired, confirm_Error, 5000, "", false, false);
			setTimeout(function(){window.location.href = "drive.html";}, 3000);
		} else if(loggedin == false){
			autoLoginCheck(response);
		} else if((response.messageCode == 429 || response.messageCode == 427 || response.messageCode == 424
				|| response.messageCode == 435 || response.messageCode == 419 || response.messageCode == 414) && $("#divlogin").is(':visible')) {
			loginsuccess(response);
		} else {
			if(!is415ExcludeURLS(postcururl)){
				loginRemoveAccount(uniqueidentifieracclist(), false);
				clearauthinlocalstorage();
				window.location.href="error.html";
			} else {
				ajaxindicatorstop();
				showconfirmmsgmainbody("You do not have permission to perform the action.", confirm_Error, 5000, "", false, false);
			}
		}
	} else if(response.messageCode == 437){
		ajaxindicatorstop();
		clearauthinlocalstorage();
		loginShowconfirmmsg(global_messages.licenseexpired, confirm_Error, 5000, "", false, false);
		setTimeout(function(){window.location.href = "drive.html";}, 3000);
...
function is415ExcludeURLS(url) {
	var isPresent = false;
	var arr = excludeurlsinit415arr.split(",");
 ...
 * @returns {Boolean}
 */
function showconfirmmsgmainbody(txt, type, duration, targetdvid, ispop, confirm)
{
	if(type == "ASK"){
...
function getdatefromutcdata(datestring, boolshowtime)
{
	if(datestring != "")
	{
		var cloudlastupdated = Date.parse(datestring);
		cloudlastupdated = getdatefromtimestamp(cloudlastupdated, boolshowtime);
...
function storeauthinlocalstorage(dataValue){
	...
	{
		if(localStorage._zt == 1 && localStorage._zy == globalvariable.defaultemailid && localStorage._zw == globalvariable.clientidlong){
		...
		localStorage.setItem("_zzs", dataValue.storageType);

		//logo path
		const random = Math.random() * 100000;
		var logo = dataValue.tempAttr10+"";
...
function invokeAdapterCall(type,posturl,base64authheader,jsoninout,jsonextraparam){
	...
	if(jsonextraparam != null && jsonextraparam != undefined){
		customheader = jsonextraparam.customheader;
	}
		...
		if(base64authheader != undefined && base64authheader != null && base64authheader != "" && base64authheader != "null" && base64authheader != "undefined") {
			$.ajax({
				...
				beforeSend: function (xhr) {
					xhr.setRequestHeader ("Authorization",  "Bearer " + base64authheader);
					if(customheader != null && customheader != undefined){
						Object.keys(customheader).forEach(function(key) {
			    			xhr.setRequestHeader (key, customheader[key]);
						});
					}
				},
...
function apiServiceInteraction_Callback(response){
	if(response != null && response != undefined){
		...
		if(!filedownload){
			...
			else if(responsecode == "432" || responsecode == "434" || responsecode == "438" || responsecode == "439")
			{
			...
			else if(responsecode == "419" || responsecode == "414")
			{
				checkErrorPage(data);
			}
			...
			else if(data.statusCode == "440" || data.statusCode == "441" || data.statusCode == "442"
				|| data.statusCode == "443" || data.statusCode == "444" || data.statusCode == "445" || data.statusCode == "446")
			{
				window[data.action](data);
			}
			...
			{
				if(window.location.href.indexOf("/drive.html") > 0) {
					if($("#divauthregeneratemodal").is(":visible")){
						window[data.action](data);
					} else {
						checkErrorPage(data);
					}
			...
			else if(data.statusCode == "401" || data.statusCode == "403" || responsecode == "404")
			{
				window[data.action](data);
			}
...
//method to set logo for each page load
function fetchLogo(){
	var logopth = defaultLogoPath;
	if (localStorage._zl != undefined && localStorage._zl != null && (localStorage._zl).indexOf("null?") < 0
			&& (localStorage._zl).indexOf("?v") != 0 && handleNullValue(localStorage._zl) != ""){
		const random = Math.random() * 100000;
		if(localStorage._zl != defaultLogoPath) logopth = cloudApiUrlACMS+ '/' +localStorage._zl;
	}

	$("#clogoimg").attr("src", logopth);
	setTimeout(function(){
		$("#clogoimg").show();
	}, 2000);
}

//check and change if logo changed for each page
function checkPageCorpLogoChange(){
	setInterval(function(){
		if(getauthtokenfromlocal() == ""){
			window.location.reload();
		}
		if(admbaseauth != getauthtokenfromlocal() && (window.location.href).indexOf("/drive.html") < 0){
			window.location.reload();
		}

		var logo = $("#clogoimg").attr("src");
		logo = logo.replace(cloudApiUrlACMS+"/", "");
		logo = logo.replace(cloudApiUrlACMS+"/", "");
		var zllogo = handleNullValue(localStorage._zl);
		if(zllogo != logo){
			fetchLogo();
		}
	}, 1000);
...
function duePaymentService(){
	var type = updateMethod;
	var posturl = cloudApiUrlACMS+paymentwebserviceurl+"/"+localStorage._zw;
	invokeAdapterCall(type, posturl, "", "", "");
}

function duePaymentServiceAfter(response){
	if(response.error == false && response.object != null){
		var expDate = response.object;
		if(expDate.includes("#PENDING")){
			var days = expDate.split("#")[0];
			$("#paymentwarningid").show();
			$(".divdriveclass").css("margin-top", "30px");
			$(".todolistid").css("margin-top", "30px");
			$("#billnotpaid").html("Payment is due on current account. The current account will be blocked within "+days+" days, if payment is not done. Please ignore, if already paid.");
		}else{
			$("#paymentwarningid").hide();
			$(".divdriveclass").css("margin", "0px");
			$(".todolistid").css("margin", "0px");
		}
	}
	fetchLogoService();
}
...
/**
 * This function use serach html text present or not
 * @param {*} selector 
 * @param {*} text 
 * @returns 
 */
...
//fetch logo service function;
function fetchLogoService(corpid){
	if(corpid == null || corpid == undefined || corpid == "null" || corpid.length == 0){
		corpid = localStorage._zw;
	}
	var jsonInput = {
		"attribute1":"fetchLogoServiceAfter",
		"attribute2":corpid,
	};
	var type = "POST";
	var posturl = cloudApiUrlACMS+"/v1/public/user/0/corp/fetchlogo";
	invokeAdapterCall(type, posturl, "", jsonInput, "");
}
// fetch logo action function;
function fetchLogoServiceAfter(response){
	if(response.error == false && response.object != null){
		const random = Math.random() * 100000;
